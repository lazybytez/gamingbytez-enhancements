...
/**
 * This class represents a time night recipe.
 * The time day recipe allows players to change the time to night using the {@link MythicAltar}.
 */
public class TimeNightAltarRecipe extends AbstractAltarRecipe {
    private final MythicAltarFeature mythicAltarFeature;

    /**
     * Constructs a new time night ritual recipe.
     *
     * @param mythicAltarFeature the feature owning the messenger this recipe sends through
     */
    public TimeNightAltarRecipe(MythicAltarFeature mythicAltarFeature) {
    ...
    public void onRecipeComplete(
        ...
        if (!world.isDayTime()) {
            this.mythicAltarFeature.getMessenger().error(player, "It is already night!");
        ...
        new LinesToCenterAltarParticleEffect(plugin, Color.BLACK).executeParticleEffect(
                altar,
                event,
                (effectPlugin, effectAltar, effectEvent) -> {
                    effectAltar.getLocation().getWorld().setTime(14000L);

                    Component body = Component.text("The time has been changed to night by ", MessagePalette.EMPHASIS)
    ...
    public Map<PedestalLocation, ItemStack> getRecipe() {
        return Map.of(
                PedestalLocation.CENTER, new ItemStack(Material.REDSTONE_TORCH),
                PedestalLocation.NORTH_WEST, new ItemStack(Material.INK_SAC),
                PedestalLocation.SOUTH_WEST, new ItemStack(Material.INK_SAC),
                PedestalLocation.NORTH_EAST, new ItemStack(Material.INK_SAC),
                PedestalLocation.SOUTH_EAST, new ItemStack(Material.INK_SAC)
        );
    }
}
